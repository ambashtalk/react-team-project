...
import {ProfileCard,ProfileContainer,ProfileImage,DetailsComponent,DetailsContainerRow, TabTitleButton, TabBodyArea} from './profile.style';
...
export const ProfilePage: FunctionComponent<profileType> = ({comp,HeaderText,activeUser}) =>{
    ...
    useEffect(() => {
        const getData = async () => {
            ...
            const result = await axios.get(url).then((response) => {
                data = response.data;
                for(var i=0;i<data.propertyAdded.length;i++) userPropertyIndex.push(data.propertyAdded[i].id);
            });
    ...
    return (
        <ProfileContainer>
            ...
            <ProfileCard>
                ...
                <div style={{display:"flex",justifyContent:"center",alignItems:"center", fontSize:"120%", margin:"2%"}}><h1>Welcome Back, {gotData ? data.firstName : " "}!</h1></div>

                
                
                <DetailsContainerRow >
                    <div style={{width:"20%"}}><h2>Name:</h2></div>
                    <DetailsComponent>{gotData ? data.firstName : ""} {gotData ? data.lastName : " "}</DetailsComponent>
                ...
                <DetailsContainerRow >
                    ...
                    <DetailsComponent>{gotData ? data.dateOfBirth : " "}</DetailsComponent>

                </DetailsContainerRow>
                <TabBodyArea>
                    <TabTitleButton onClick={toggle} title="Edit Profile">Edit Profile</TabTitleButton>
                    <Modal isShown={isShown} hide={toggle} headerText={HeaderText} modalContent={<ConfirmationModal message={comp} />}/>
